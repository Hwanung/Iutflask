...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'J etais challenger en S9 sur LoL'
...
def create():
    if request.method == 'POST':
        ...
        if not SepalLengthCm:
            flash('SepalLengthCm is required!')
        else:
            conn = get_db_connection()
            conn.execute('INSERT INTO posts (SepalLengthCm,SepalWidthCm,PetalLengthCm,PetalWidthCm,Species ) VALUES (?, ?, ?, ?, ?)',
...
def delete(id):
    ...
    conn.commit()
    conn.close()
    flash('"{}" il est plus la lala la la'.format(post['id']))
    return redirect(url_for('index'))
